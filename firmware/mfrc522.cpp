...
{
    asm volatile("nop \n nop \n nop");
    gpio_put(PIN_RFID_SS, 0);  // Active low
    asm volatile("nop \n nop \n nop");
}
...
void MFRC522::PCD_WriteRegister(
  ...
  uint8_t buf[2];
  buf[0] = reg & ~(READ_BIT);  // remove read bit as this is a write
  buf[1] = value;
  cs_select();
...
void MFRC522::PCD_WriteRegister(
  ...
  uint8_t addr = reg & ~(READ_BIT);  // remove read bit as this is a write
  cs_select();
  spi_write_blocking(spi_default, &addr, 1);
  spi_write_blocking(spi_default, values, count);
  vTaskDelay(0);
  cs_deselect();
  vTaskDelay(0);
#if DEBUG == 1
...
void MFRC522::PCD_ReadRegister(
  ...
  {
    spi_read_blocking(spi_default, addr, values, count);
  }
...
MFRC522::StatusCode MFRC522::PCD_CalculateCRC(
  ...
  size_t length,    ///< In: The number of bytes to transfer.
...
void MFRC522::PCD_Init() {
  ...
  bi_decl(bi_3pins_with_func(PIN_MISO, PIN_MOSI, PIN_SCK, GPIO_FUNC_SPI));  // For picotool
  gpio_put(PIN_RFID_SS, 1);
  bi_decl(bi_1pin_with_name(PIN_RFID_SS, "SPI CS")); // For picotool

  // Hard reset
  gpio_put(PIN_RFID_RESET, 0);
  vTaskDelay(1);
  gpio_put(PIN_RFID_RESET, 1);
  vTaskDelay(10);
  ...
  PCD_WriteRegister(ModeReg, 0x3D);    // CRC valor inicial de 0x6363

  PCD_ClearRegisterBitMask(Status2Reg, 0x08); //MFCrypto1On=0
  PCD_WriteRegister(RxSelReg, 0x86);   //RxWait = RxSelReg[5..0]
  PCD_WriteRegister(RFCfgReg, 0x7F);     //RxGain = 48dB

  PCD_AntennaOn();    //Open the antenna
#endif
...
bool MFRC522::PCD_PerformSelfTest() {
  ...
  for (uint8_t i = 0; i < 64; i++) {
    if (result[i] != reference[i]) {
      printf("Index %d Result: %x Reference: %x\n", i, result[i], reference[i]);
      return false;
    }
  }
...
void MFRC522::PCD_SoftPowerUp(){
  ...
  uint32_t deadline = 500;// create timer for timeout (just in case) 
  
  while(deadline-- > 0){ // set timeout to 500 ms 
...
MFRC522::StatusCode MFRC522::PCD_TransceiveData(
  ...
  size_t *backLen,      ///< In: Max number of uint8_t to write to *backData. Out: The number of bytes returned.
  ...
  while (!uidComplete) {
    ...
    while (!selectDone) {
      ...
      if (currentLevelKnownBits >= 32) { // All UID bits in this Cascade Level are known. This is a SELECT.
        //printf("SELECT: currentLevelKnownBits="); printf(currentLevelKnownBits, DEC);
        ...
        //printf("ANTICOLLISION: currentLevelKnownBits="); printf(currentLevelKnownBits, DEC);
...
MFRC522::StatusCode MFRC522::MIFARE_Read(
  ...
  size_t *bufferSize   ///< Buffer size, at least 18 bytes. Also number of bytes returned if STATUS_OK.
) {
  MFRC522::StatusCode result;
...
MFRC522::StatusCode MFRC522::MIFARE_Write(
  ...
  size_t bufferSize  ///< Buffer size, must be at least 16 bytes. Exactly 16 bytes are written.
) {
  MFRC522::StatusCode result;
...
MFRC522::StatusCode MFRC522::MIFARE_Ultralight_Write(
  ...
  size_t bufferSize  ///< Buffer size, must be at least 4 bytes. Exactly 4 bytes are written.
) {
  MFRC522::StatusCode result;
...
MFRC522::StatusCode MFRC522::PCD_MIFARE_Transceive(
  ...
  size_t sendLen,       ///< Number of bytes in sendData.
  ...
  // Transceive the data, store the reply in cmdBuffer[]
  uint8_t waitIRq = 0x30;    // RxIRq and IdleIRq
  size_t cmdBufferSize = sizeof(cmdBuffer);
  uint8_t validBits = 0;
...
bool MFRC522::MIFARE_OpenUidBackdoor(bool logErrors) {
  ...
  size_t received = sizeof(response);
 ...
 */
bool MFRC522::MIFARE_SetUid(uint8_t *newUid, size_t uidSize, bool logErrors) {
...
//        PICC_WakeupA(atqa_answer, &atqa_size);
  ...
  size_t byteCount = sizeof(block0_buffer);
...
bool MFRC522::PICC_IsNewCardPresent() {
  ...
  size_t bufferSize = sizeof(bufferATQA);
